...
@pytest.mark.parametrize ('line, result', [("дом", "дом"),
("крыша", "крыша"),
("123", "123")
])
...
("", ""),# должен падать но проходит тест
(" ", ""), #также должен падать но проходит
(None,None),
])
...
@pytest.mark.parametrize ('line, deli, result ', [(None, None, None),])            
...
@pytest.mark.parametrize ('line, simbol, result', [("", "", True),
(" ", " ", True), 
("Дом", "П", False)
])                                            
...
@pytest.mark.parametrize ('line, simbol, result', [("Привет", "П", "ривет"),
("123", "3", "12")
])                                            
...
def negative_test_delete_symbol( line, simbol, result ):
    stringutils = StringUtils()
    res = stringutils.delete_symbol(line, simbol)
    assert res == result
...
@pytest.mark.parametrize ('line, simbol, result', [("Текст", "Т", True),
("123", "1", True), ("14 мая 2023", "1", True)])                                           
...
@pytest.mark.parametrize ('line, simbol, result', [("Терем", "м", True),
("123", "3", True),
("tree", "t", False)
])                                           
...
@pytest.mark.parametrize ('line, result', [("123", False), 
("Дом", False),
(",,", False)
])                                           
...
@pytest.mark.parametrize ('line, simbol, result', [([1, 2, 3, 4], ",", "1,2,3,4" ),
(["Солн", "це"], ":", "Солн:це"),
([], ",", ""),
([" "], ",", " ")
])
...
@pytest.mark.parametrize ('line, simbol, result', [(None, ",", None), ("213", ",", "2,1,3")
